...
func main() {
	...
	fmt.Println("\n══════ SCENARIO H: deference — Commit picks, the user still consents ══════")
	now = time.Now()
	...
	check("Commit made the pick (didn't bounce 'which one?' back)", sender.selfSaid("i'd take"))
	check("the pick comes with a reason", sender.selfSaid("earliest") || sender.selfSaid("sits right next to"))
	check("surfaced for consent — NOT booked", sessionState(db) == "reply_surfaced")
	check("nothing sent to the counterpart yet", !sender.selfSaid("booked"))
	check("asks for the one word", sender.selfSaid("'yes' to book it"))
	userSelf(m, db, "yes", now.Add(6*time.Minute))
	check("'yes' books the picked slot", sessionState(db) == "closed/none")
	check("counterpart confirmed only after consent", sender.selfSaid("booked"))
