...
// cleanupPendingClosures lazily drops pending rows whose commitment was
// resolved or dismissed through another path (Done button, extraction,
// staleness) so the confirm list never shows already-closed items.
...
// GetPendingClosures returns detections awaiting confirmation, newest first,
// joined with the commitment for display.
...
// RejectPendingClosure keeps the commitment open, records the rejected
// detection as future training data, and clears the pending row.
func (db *DB) RejectPendingClosure(commitmentID string) error {
	_, err := db.conn.Exec(`
		INSERT INTO closure_rejections (commitment_id, confidence, evidence, closure_type, detected_at, rejected_at)
		SELECT commitment_id, confidence, evidence, closure_type, detected_at, ?
